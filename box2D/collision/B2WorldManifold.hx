...
class B2WorldManifold
{
	 ...
	 */
	public function initialize(manifold:B2Manifold, xfA:B2Transform, radiusA:Float, xfB:B2Transform, radiusB:Float):Void
	{
			...
			case FACE_A:
				{
					...
					for (i in 0...manifold.m_pointCount)
					{
						...
						m_points[i].x = clipPointX
							+ 0.5 * (radiusA - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusB) * normalX;
						m_points[i].y = clipPointY
							+ 0.5 * (radiusA - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusB) * normalY;
					}
				}
			...
			case FACE_B:
				{
					...
					for (i in 0...manifold.m_pointCount)
					{
						...
						// m_points[i] = 0.5f * (cA + cB);
						m_points[i].x = clipPointX
							+ 0.5 * (radiusB - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusA) * normalX;
						m_points[i].y = clipPointY
							+ 0.5 * (radiusB - (clipPointX - planePointX) * normalX - (clipPointY - planePointY) * normalY - radiusA) * normalY;
					}
				}
		}
	}
