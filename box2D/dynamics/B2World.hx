...
class B2World
{
	...
	public function queryShape(callbackMethod:B2Fixture->Dynamic, shape:B2Shape, transform:B2Transform = null):Void
	{
		...
		function worldQueryWrapper(proxy:Dynamic):Bool
		{
			...
			if (B2Shape.testOverlap(shape, transform, fixture.getShape(), fixture.getBody().getTransform())) return callbackMethod(fixture);
			return true;
		}
	...
	public function solveTOI(step:B2TimeStep):Void
	{
		...
		var island:B2Island = m_island;
		island.initialize(m_bodyCount, B2Settings.b2_maxTOIContactsPerIsland, B2Settings.b2_maxTOIJointsPerIsland, null, m_contactManager.m_contactListener,
			m_contactSolver);
		...
		while (true)
		{
			...
			while (c != null)
			{
				...
				else
				{
					...
					if ((bA.getType() != DYNAMIC_BODY || bA.isAwake() == false) && (bB.getType() != DYNAMIC_BODY || bB.isAwake() == false))
					{
						c = c.m_next;
						continue;
					}
